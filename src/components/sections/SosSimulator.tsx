...
export default function SosSimulator() {
  ...
  // Handle the long press simulation
  useEffect(() => {
    ...
    if (stage === "pressing") {
      interval = setInterval(() => {
        setProgress((p) => {
          ...
          return p + 5; // 2 seconds to reach 100% (2000ms / 50ms * 5 = 200%) so 100 / 5 = 20 ticks. 20 * 50ms = 1000ms. Wait, 100/5 = 20. 20 * 50 = 1000ms (1 second).
          // Let's make it 2 seconds (2000ms). 2000 / 50 = 40 ticks. 100 / 40 = 2.5
        });
      }, 50);
    } else if (stage === "idle") {
      setProgress(0);
  ...
  // Handle automatic transition from transmitting to alerted
  useEffect(() => {
    ...
    <section className="py-32 relative bg-[#0A0A0A] overflow-hidden">
      ...
      <div className="section-container">
        ...
        <div className="flex flex-col lg:flex-row gap-12 items-center justify-center max-w-5xl mx-auto">
          ...
          <div className="w-full lg:w-1/3 glass p-8 rounded-3xl border border-white/10 relative flex flex-col items-center">
            <h3 className="text-sm font-mono text-gray-500 uppercase tracking-widest mb-12">Hardware Layer</h3>
            ...
            <div className="w-full bg-black/50 p-4 rounded-xl border border-white/5 font-mono text-xs">
          ...
          {/* Cloud / App Panel */}
          <div className="w-full lg:w-1/2 glass p-8 rounded-3xl border border-white/10 relative overflow-hidden">
            <h3 className="text-sm font-mono text-gray-500 uppercase tracking-widest mb-6">Cloud & Contacts</h3>
              ...
              <AnimatePresence mode="wait">
                {stage === "idle" || stage === "pressing" ? (
                  ...
                  >
                    ...
                    <p className="font-mono text-sm tracking-widest animate-pulse">TRANSMITTING PAYLOAD...</p>
                  </motion.div>
                ) : (
                  <motion.div 
                    key="alerted"
                  ...
                  >
                    <div className="bg-red-alert/10 border border-red-alert/30 p-4 rounded-xl flex items-start gap-4 mb-4">
                      ...
                      <div>
                        <h4 className="text-red-alert font-bold">CRITICAL ALERT DISPATCHED</h4>
                        <p className="text-sm text-gray-400 mt-1">SMS and App notifications sent to 3 emergency contacts.</p>
                      </div>
                    </div>
                    ...
                    <button onClick={reset} className="mt-4 text-xs text-gray-500 hover:text-white underline text-center w-full transition-colors">
                      Reset Simulator
                    </button>
                  </motion.div>
                )}
